...
int main() {
  Node* scoreList = NULL; // Inicializa a lista vazia
...
// Insere um novo score no final da lista
...
// Mostra a lista de scores no terminal
void printScores(Node* head) {
    printf("\nPontuações registradas nesta execução:\n");
    Node* temp = head;
    while (temp != NULL) {
        printf(" - %d pontos\n", temp->score);
        temp = temp->next;
